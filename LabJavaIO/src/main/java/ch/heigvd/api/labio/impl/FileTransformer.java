...
public class FileTransformer {
  ...
  public void transform(File inputFile) {
    ...
    try {
      ...
      OutputStreamWriter outputStreamWriter = new OutputStreamWriter(new FileOutputStream(inputFile.getPath()+".out"), StandardCharsets.UTF_8);
      FileReader fileReader = new FileReader(inputFile, StandardCharsets.UTF_8);

      for (int iterator = fileReader.read();iterator != -1; iterator = fileReader.read()) {
        outputStreamWriter.write(lineNumberingCharTransformer.transform(upperCaseCharTransformer.transform(String.valueOf(Character.toChars(iterator)))));
      }
